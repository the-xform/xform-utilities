...
/*
 * A library can be used on non-Windows platforms even if one class is Windows-only, as long as:
 * - The Windows-only APIs are isolated
 * - They are never executed on non-Windows platforms
 * - Call sites are properly guarded
 * It will fail if:
 * - Windows-only types are referenced unconditionally
 * - The JIT tries to load Windows-only APIs at runtime
 * 
 * [SupportedOSPlatform("windows")] attribute:
 * - Does NOT prevent the assembly from loading on Linux/macOS
 * - Produce compile-time warnings when called from cross-platform code
 * - Tell analyzers that runtime OS checks are required
 * - Is advisory, not enforcement
 * 
 * Safe scenario:
 *  SecurityUtilities.dll
 *  ├── SecurityUtilities (Windows-only, DPAPI, RSA CAPI)
 *  ├── StringUtilities (cross-platform)
 *  ├── MathUtilities (cross-platform)
 *  
 *  - The assembly loads fine on Linux/macOS
 *  - Cross-platform classes work normally
 *  - Windows-only class is never touched
 *  
 *  Invoking Windows-only class/methods on a non-windows platform 
 *  will generate warnings at compile time and throw exception at 
 *  runtime on non-Windows platforms
 */
